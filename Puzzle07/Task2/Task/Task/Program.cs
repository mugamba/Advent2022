...
namespace Task
{
    class Program
    {
        ...
        public static List<long> _matchingDirectories = new List<long>();
        ...
        static void Main(string[] args)
        {
            ...
            var unusedSpace = 70000000 - _tree.DirSize;
            var minimumSizeTodelete =  (long)30000000 - unusedSpace;

            FindDirectory(_tree, minimumSizeTodelete);
    
            Console.WriteLine("Result is {0}", _matchingDirectories.Min());
            Console.ReadKey(); 
        }

        public static void FindDirectory(Dir current,long minimumSizeTodelete)
        {
            if (current.DirSize >= minimumSizeTodelete)
            {
                _matchingDirectories.Add(current.DirSize);
            }

            foreach (var child in current.Dirs)
            {
                FindDirectory(child, minimumSizeTodelete);
            }
        }
    }
